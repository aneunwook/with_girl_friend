...
export const AuthProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (token) {
            axiosInstance.defaults.headers.common["Authorization"] = `Bearer ${token}`;  // ✅ token 변경 시 axiosInstance 헤더 업데이트
            console.log("✅ useEffect에서 axiosInstance 기본 헤더 설정 완료:", axiosInstance.defaults.headers.common["Authorization"]);
  ...
  const login = async (userData) => {
    ...
    try{
        ...
        if (data.token) {
            ...
            console.log("✅ localStorage에 저장된 토큰:", localStorage.getItem("token")); // 🔥 확인용 로그

            setToken(data.token);

            // ✅ axiosInstance의 기본 헤더에 즉시 반영
            axiosInstance.defaults.headers.common["Authorization"] = `Bearer ${data.token}`;
            console.log("✅ axiosInstance 기본 헤더에 저장된 토큰:", axiosInstance.defaults.headers.common["Authorization"]);

            console.log("저장된 토큰:", data.token);

            fetchUserProfile(data.token); // ✅ 로그인 후 사용자 정보 가져오기!
        }else{
